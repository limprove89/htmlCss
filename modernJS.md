...
- 브라우저는 html 코드를 내부적으로 정리
- html 태그 안에 head 태그, 각 태그를 객체로 생성하여 연결 (부모-자식관계)
- body 태그와 기타 존재하는 태그들을 모두 객체로 생성하여 연결
- 각 객체에는 모든 해당 태그들의 속성 값들이 각각 저장
- 브라우저는 이런 트리 구조로 표현하고, 그 정보를 시각화