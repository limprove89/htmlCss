...
Mixin은 style(속성:값)을 반환하지만, 함수는 연산된 특정 값을 @return 지시어를 통해 반환한다.  
또한 사용 방법도 Mixin은 @include로 사용하며, 함수는 함수이름()으로 바로 사용한다.
...
함수는 별도의 @지시어 없이 사용하기에 내가 지정한 함수와 내장 함수(Built-in-Functions)의 이름이 충돌할 수 있다.  
따라서 내가 지정하는 함수에는 별도의 접두어를 붙이는것을 추천한다.

### 조건과 반복 (Control Directive / Expressions)

#### if(함수)

조건의 값(true, false)에 따라 두 개의 표현식 중 하나만 반환  
조건이 true이면 앞에 표현식을, false이면 뒤에 표현식을 실행한다.
...
for문과 유사하며, @for from은 through와 to 형식으로 나뉜다.  
보통 변수는 관례적으로 $i 혹은 $index를 사용한다.
